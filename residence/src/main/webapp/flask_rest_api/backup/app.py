...
def recommend():
    ...
    df = pd.read_excel(path + "web/functions/price_test.xlsx", index_col=0)
    ...
    try:
        ...
        if '선택' in user_searched:
            print(user_searched)

            user_searched = user_searched.replace("선택", '')
            user_searched = user_searched.replace
            print(user_searched)
